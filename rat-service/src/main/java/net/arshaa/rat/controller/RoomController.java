...
// Dependency Injection-Injecting those objects into controller layer
    ...
    private AllBuildingRepository buildingsRepo;
		              ...
		              getRooms.get().forEach(room->{
		                ...
		                if(getBeds.isPresent())
		                {
		                  getBeds.get().forEach(bed -> {
		                    ...
		                    newBed.setBedStatus(bed.isBedStatus());
		                    newBed.setFloorId(bed.getFloorId());
		                    newBed.setRoomId(bed.getRoomId());listbed.add(newBed);
		                  });
		                }
		              });
		            }
		  ...
		  //Get mapping
			   ...
			   /* @RequestMapping(path ="/getRoomsByFloorId/{id}")
			    public ResponseEntity<java.util.List<Room>> getById(@PathVariable Integer id) {

			    Optional<List<Room>> listRooms = bedrepo.findById(id);
			    return new ResponseEntity<>(listRooms.get(), HttpStatus.OK);
			    }*/
			    
			    
			  /*  @PostMapping(path ="/add")
			    public ResponseEntity<Room> addBed(@RequestBody Room newRoom) {

			   Room room = bedrepo.save(newRoom);
	 ...
	 //Get beds by roomid
		   ...
		   /* 
		    @RequestMapping(path ="/getBedsByRoomId/{id}")
			public ResponseEntity<java.util.List<Bed>> getByRoomId(@PathVariable Integer id) {
				
				List<Bed> listFloor = bedrepo.findByroomId(id);
